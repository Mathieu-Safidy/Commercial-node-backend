...
class UserService {
    static async createUserSave(email, username, password, idProfil) {
    ...
    static async getidProfilByIdProfilFront(idProfilFront) { 
        if (idProfilFront == 1) {
    ...
    static async generatePassword(length = 10) {
        return crypto
            .randomBytes(length)
            .toString("base64")
            .replace(/[^a-zA-Z0-9]/g, "")
